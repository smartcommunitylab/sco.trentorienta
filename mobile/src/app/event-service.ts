...
export class EventService {
    ...
    getEvents(from = 0, to = 65535): Promise<eventType[]> {
        return this.http.get(this.serverUrl + 'events',
            ...
            .then(response => {
                var tempEvents: eventType[];
                var modifiedEvents: eventType[] = [];
                tempEvents = response.json().content;
                for (var t = 0; t < tempEvents.length; t++) {
                    var modifiedEvent = this.preprocessEventImage(tempEvents[t]);
    ...
    getEvent(id: string): Promise<eventType> {
        ...
        return this.http.get(url, {
            ...
            .toPromise()
            .then(response => {
                var tempEvent: eventType;
                return this.preprocessEventImage(response.json());
            })
            .catch(this.handleError);
    }

    preprocessEventImage(tempEvent) {
        var imageLink = tempEvent.image;
        if (imageLink) {
            var iconImage = imageLink.substr(0, tempEvent.image.lastIndexOf("."));
            iconImage = iconImage + "_medium" + imageLink.substr(imageLink.lastIndexOf("."), imageLink.length);
            tempEvent.imageThumb = iconImage;
        }
        return tempEvent;
    }
